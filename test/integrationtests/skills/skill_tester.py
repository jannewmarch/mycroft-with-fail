    ...
    def run(self, loader):
        ...
        while not evaluation_rule.all_succeeded():
            try:
                ...
                if event.type == 'skill.handler.complete':
...
class EvaluationRule(object):
    ...
    def __init__(self, test_case, skill=None):
        ...
        # Add rules from expected_response sequence
        # Accepts a string or a list of multiple strings
        if isinstance(test_case.get('expected_response_sequence', None), str):
            self.rule.append(['match', 'utterance',
                              str(test_case['expected_response_sequence'])])
        elif isinstance(test_case.get('expected_response_sequence', None), list):
            texts = test_case['expected_response_sequence']
            rules = [['match', 'utterance', str(r)] for r in texts]
            self.rule.append(['sequence'] + rules)
    ...
    def _partial_evaluate(self, rule, msg):
        ...
        if rule[0] == 'sequence':
            ...
            if not self._partial_evaluate(i, msg):
                ...
                # move to the next sequence elmt
                # but don't append 'succeeded'
                # That's only done if no more elemts left to check (len()==1)
                rule[1:] = rule[2:]
            if len(rule) > 1:
                return True
